...
fn main() {
    ...
    let mut clean_tiles = vec![vec![b'.'; lines[0].len()]; lines.len()];

    clean_tiles[position[0] as usize][position[1] as usize] = b'S';
    ...
    for direction in DIRECTIONS {
        ...
        if ends.iter().any(|dir| *dir == reverse) {
            ...
            while prospective_tile != b'S' {
                ...
                clean_tiles[prospective_position[0] as usize][prospective_position[1] as usize] = prospective_tile;
    ...
    let finished: Vec<_> = clean_tiles.iter().map(|line| std::str::from_utf8(line).unwrap()).collect();
    let finished = finished.join("\n");
