...
fn main() {
    ...
    while let Some(current) = pq.pop() {
        let current_dir = current.0.1;
        let current = current.0.0;
        if goal == current {
            println!("GOAL");
            break;
        }
        ...
        for dir in DIRECTIONS {
            ...
            if grid[next] == u8::MAX { continue }


            // let mut tripled = true;
            // let mut prev = current_dir;
            // for _ in 0..2 {
            //     if prev != dir {
            //         tripled = false;
            //         break;
            //     }
            //     prev = came_from[&current].unwrap().1;
            // }
            // if tripled { continue }
            ...
            if !cost_so_far.contains_key(&next) || new_cost < cost_so_far[&next] {
                ...
                let priority = -(new_cost as isize);
